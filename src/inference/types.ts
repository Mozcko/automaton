...
export const STATIC_MODEL_BASELINE: Omit<ModelEntry, "lastSeen" | "createdAt" | "updatedAt">[] = [
  {
    ...
    provider: "nim",
    displayName: "Llama 3.1 8B Instruct (NIM)",
    tierMinimum: "critical",
    costPer1kInput: 13.2,
    costPer1kOutput: 39.6,
    maxTokens: 8192,
...
export const DEFAULT_ROUTING_MATRIX: RoutingMatrix = {
  high: {
    agent_turn: { candidates: ["deepseek-coder"], maxTokens: 8192, ceilingCents: -1 },
    heartbeat_triage: { candidates: ["deepseek-coder"], maxTokens: 2048, ceilingCents: 5 },
    safety_check: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: 20 },
    summarization: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: 15 },
    planning: { candidates: ["deepseek-coder"], maxTokens: 8192, ceilingCents: -1 },
  },
  normal: {
    agent_turn: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: -1 },
    heartbeat_triage: { candidates: ["deepseek-coder"], maxTokens: 2048, ceilingCents: 5 },
    safety_check: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: 10 },
    summarization: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: 10 },
    planning: { candidates: ["deepseek-coder"], maxTokens: 4096, ceilingCents: -1 },
  },
  low_compute: {
    agent_turn: { candidates: ["llama3.1:latest"], maxTokens: 4096, ceilingCents: 10 },
    heartbeat_triage: { candidates: ["llama3.1:latest"], maxTokens: 1024, ceilingCents: 2 },
    safety_check: { candidates: ["llama3.1:latest"], maxTokens: 2048, ceilingCents: 5 },
    summarization: { candidates: ["llama3.1:latest"], maxTokens: 2048, ceilingCents: 5 },
    planning: { candidates: ["llama3.1:latest"], maxTokens: 2048, ceilingCents: 5 },
  },
  critical: {
    agent_turn: { candidates: ["llama3.1:latest"], maxTokens: 2048, ceilingCents: 3 },
    heartbeat_triage: { candidates: ["llama3.1:latest"], maxTokens: 512, ceilingCents: 1 },
    safety_check: { candidates: ["llama3.1:latest"], maxTokens: 1024, ceilingCents: 2 },
